...
# randomly generate some data, just use it for debugging
def random_generate(N= 7575, dx= 1200, T= 10):
    X_list = []
    A_list = []
    C_list = []
    Y1_true_list = []
    ...
    id_all = list(range(N))
    idx_trn = random.sample(id_all, int(0.6 * N))
    rest = list(set(id_all) - set(idx_trn))
    idx_tst = random.sample(rest, int(0.2 * N))
    idx_val = list(set(id_all) - set(idx_trn) - set(idx_tst))
    ...
    for t in range(T):
        X = torch.tensor(np.random.random((N, dx)), dtype = torch.float32)
        A = np.random.randint(0,2,size=[N, N])
        C = np.random.randint(0,2,size=N)
        Y1_true = np.random.random(N)
        Y0_true = np.random.random(N)

        dense_A_list.append(torch.tensor(A))
        A = sp.csr_matrix(A)
        sparse_A_list.append(A)

        C = torch.LongTensor(C)
        Y1_true = torch.FloatTensor(Y1_true)
        Y0_true = torch.FloatTensor(Y0_true)

        X_list.append(X)
        C_list.append(C)
        Y1_true_list.append(Y1_true)
        Y0_true_list.append(Y0_true)
    ...
    return X_list, sparse_A_list, dense_A_list, C_list, Y1_true_list, Y0_true_list, idx_trn_list, idx_val_list, idx_tst_list
